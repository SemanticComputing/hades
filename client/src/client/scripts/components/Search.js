...
export default class Search extends React.Component {
  ...
  onInputChange(text, uri) {
    this.setState({ query: text, queryUri: uri });
  }
  ...
  query() {
    ...
    return new Promise((resolve, reject) => {
      ...
      return webApi.searchByEntity(this.state.query, this.state.queryUri).then((results) => {
          ...
          <form onSubmit={this.onSubmit}>
            ...
            <div className="query-doc-category" style={{display: 'inline-block'}}>
              <FormControl value={this.state.docCategory} onChange={this.handleDocCategoryChange} componentClass="select" placeholder="Valitse">
                <option key={0} value="news">Uutiset</option>
              </FormControl>
            </div>
